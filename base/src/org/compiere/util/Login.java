 ...
 *         <li>BF [
 *         <a href="https://github.com/adempiere/adempiere/issues/1935">1935</a>
 *         ] Add default role
...
public class Login {
	 ...
	 * @return Array of Role KeyNamePair or null if error The error (NoDatabase,
	 *         UserPwdError, DBLogin) is saved in the log
	 */
	 ...
	 * @return role array or null if in error. The error (NoDatabase, UserPwdError,
	 *         DBLogin) is saved in the log
	 */
	...
	public int getAuthenticatedUserId(String app_user, String app_pwd) {
		...
		// adaxa-pb: try to authenticate using hashed password -- falls back to plain
		// text/encrypted
		String sql = "SELECT AD_User_ID, Password, Salt FROM AD_User " + "WHERE COALESCE(LDAPUser," + userLogin
				+ ") = ? AND" + " EXISTS (SELECT 1 FROM AD_User_Roles ur"
				...
				+ " EXISTS (SELECT 1 FROM AD_Client c" + "         WHERE c.AD_Client_ID=AD_User.AD_Client_ID"
				+ "         AND c.IsActive='Y') " + "AND AD_User.IsActive='Y' ";
		if (loginWithValue) {
	...
	private KeyNamePair[] getRoles(String app_user, String app_pwd, boolean force) {
		...
		//
		StringBuffer sql = new StringBuffer("SELECT u.AD_User_ID, r.AD_Role_ID,r.Name,").append(" u.ConnectionProfile ")
				.append("FROM AD_User u")
				.append(" INNER JOIN AD_User_Roles ur ON (u.AD_User_ID=ur.AD_User_ID AND ur.IsActive='Y')")
				.append(" INNER JOIN AD_Role r ON (ur.AD_Role_ID=r.AD_Role_ID AND r.IsActive='Y') ")
				.append("WHERE u.AD_User_ID = ?").append(" AND u.IsActive='Y'")
	...
	public KeyNamePair[] getClients(KeyNamePair role) {
		...
		String sql = "SELECT DISTINCT r.UserLevel, r.ConnectionProfile, " // 1/2
				...
				+ "FROM AD_Role r" + " INNER JOIN AD_Client c ON (r.AD_Client_ID=c.AD_Client_ID) "
	 ...
	 * Load Organizations.
	 * <p>
	 * Sets Client info in context and loads its organization, the role has access
	 * to
	 * 
	...
	public KeyNamePair[] getOrgs(KeyNamePair client) {
		...
		String sql = "SELECT o.AD_Org_ID,o.Name,o.IsSummary " // 1..3
				...
				+ " INNER JOIN AD_Org o ON (c.AD_Client_ID=o.AD_Client_ID OR o.AD_Org_ID=0) " + "WHERE r.AD_Role_ID=?" // #1
				+ " AND c.AD_Client_ID=?" // #2
				+ " AND o.IsActive='Y' AND o.IsSummary='N'" + " AND (r.IsAccessAllOrgs='Y' "
		...
		try {
			...
			retValue = new KeyNamePair[list.size()];
			list.toArray(retValue);
			log.fine("Client: " + client.toStringX() + ", AD_Role_ID=" + AD_Role_ID + ", AD_User_ID=" + AD_User_ID
		...
		if (retValue == null || retValue.length == 0) {
			log.log(Level.WARNING, "No Org for Client: " + client.toStringX() + ", AD_Role_ID=" + AD_Role_ID
	...
	private void getOrgsAddSummary(ArrayList<KeyNamePair> list, int Summary_Org_ID, String Summary_Name, MRole role) {
		...
		String sql = "SELECT AD_Client_ID, AD_Org_ID, Name, IsSummary FROM AD_Org "
				...
				+ " WHERE AD_Tree_ID=? AND Parent_ID=? AND IsActive='Y') " + "ORDER BY Name";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			pstmt = DB.prepareStatement(sql, null);
	...
	public KeyNamePair[] getWarehouses(KeyNamePair org) {
		...
		ArrayList<KeyNamePair> list = new ArrayList<KeyNamePair>();
		KeyNamePair[] retValue = null;
		String sql = "SELECT M_Warehouse_ID, Name FROM M_Warehouse " + "WHERE AD_Org_ID=? AND IsActive='Y' " + " AND "
				+ I_M_Warehouse.COLUMNNAME_IsInTransit + "='N' " // do not show in tranzit warehouses - teo_sarca [
																	// 2867246 ]
				+ "ORDER BY Name";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	 ...
	 * <p>
	 * Sets Org info in context and loads relevant field from - AD_Client/Info, -
	 * C_AcctSchema, - C_AcctSchema_Elements - AD_Preference
	 * <p>
	 ...
	 */
	public String loadPreferences(KeyNamePair org, KeyNamePair warehouse, java.sql.Timestamp timestamp,
			String printerName) {
		...
		String sql = "SELECT * " + "FROM C_AcctSchema a, AD_ClientInfo c "
				+ "WHERE a.C_AcctSchema_ID=c.C_AcctSchema1_ID " + "AND c.AD_Client_ID=?";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			// Accounting Elements
			sql = "SELECT ElementType " + "FROM C_AcctSchema_Element " + "WHERE C_AcctSchema_ID=?"
					+ " AND IsActive='Y'";
			pstmt = DB.prepareStatement(sql, null);
			...
			sql = "SELECT Attribute, Value, AD_Window_ID " + "FROM AD_Preference "
					+ "WHERE AD_Client_ID IN (0, @#AD_Client_ID@)" + " AND AD_Org_ID IN (0, @#AD_Org_ID@)"
					+ " AND (AD_User_ID IS NULL OR AD_User_ID=0 OR AD_User_ID=@#AD_User_ID@)" + " AND IsActive='Y' "
			...
			sql = "SELECT t.TableName, c.ColumnName " + "FROM AD_Column c "
					+ " INNER JOIN AD_Table t ON (c.AD_Table_ID=t.AD_Table_ID) "
					+ "WHERE c.IsKey='Y' AND t.IsActive='Y'" + " AND EXISTS (SELECT * FROM AD_Column cc "
	...
	private X_s_day getToday() {
		X_s_day day = null;
		String sql = "select * FROM adempiere.get_today";
	...
	private void loadDefault(String TableName, String ColumnName) {
		if (TableName.startsWith("AD_Window") || TableName.startsWith("AD_PrintFormat")
	...
	/**
	 * Batch Login using Ini values <code>
