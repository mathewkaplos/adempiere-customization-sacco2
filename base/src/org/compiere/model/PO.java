 ...
 *         <li>BF [ 1704828 ] PO.is_Changed() and PO.is_ValueChanged are not
 *         consistent
 ...
 */
public abstract class PO implements Serializable, Comparator, Evaluatee, Cloneable {
	/**
	 * 
	 */
	...
	/**
	 * Trifon - Indicates that this record is created by replication
	 * functionality.
	 */
	private boolean m_isReplication = false;

	/**
	 * Direct load, e.g. from migration. Do not overwrite assigned ID with
	 * generated one, or fire triggers
	 */
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("PO[").append(get_WhereClause(true)).append("]");
		return sb.toString();
	} // toString
	...
	/**
	 * Return new - old. - New Value if Old Value is null - New Value - Old
	...
	protected final boolean set_Value(String ColumnName, Object value) {
		...
		if (ColumnName.endsWith("_ID") && value instanceof String) {
			// Convert to Integer only if info class is Integer - teo_sarca [
			// 2859125 ]
	...
	/**
	 * Set Value if updteable and correct class. (and to NULL if not mandatory)
	 * 
	 * @param index
	 *            index
	 * @param value
	...
	protected final boolean set_Value(int index, Object value) {
		...
		//
		// globalqss -- Bug 1618469 - is throwing not updateable even on new
		// records
		...
		if (value == null) {
			...
			// added by vpj-cd
			// To solve BUG [ 1618423 ] Set Project Type button in Project
			// window throws warning
			// generated because C_Project.C_Project_Type_ID is defined as
			// button in dictionary
			// although is ID (integer) in database
				...
				try {
					...
					log.log(Level.SEVERE, ColumnName + " - Class invalid: " + value.getClass().toString()
							+ ", Should be " + p_info.getColumnClass(index).toString() + ": " + value);
					return false;
				}
			else {
				log.log(Level.SEVERE, ColumnName + " - Class invalid: " + value.getClass().toString() + ", Should be "
						+ p_info.getColumnClass(index).toString() + ": " + value);
				return false;
			}
			// Validate (Min/Max)
			...
			if (p_info.getColumn(index).DisplayType == DisplayType.List
					&& p_info.getColumn(index).AD_Reference_Value_ID > 0 && value instanceof String) {
				if (MRefList.get(getCtx(), p_info.getColumn(index).AD_Reference_Value_ID, (String) value,
						get_TrxName()) != null)
					;
				else {
					StringBuffer validValues = new StringBuffer();
					for (ValueNamePair vp : MRefList.getList(getCtx(), p_info.getColumn(index).AD_Reference_Value_ID,
							false))
						validValues.append(" - ").append(vp.getValue());
					throw new IllegalArgumentException(ColumnName + " Invalid value - " + value + " - Reference_ID="
							+ p_info.getColumn(index).AD_Reference_Value_ID + validValues.toString());
				}
			}
		...
		// FR 2962094 Fill ProcessedOn when the Processed column is changing
		// from N to Y
	...
	/*
	 * FR 2962094 - Finish implementation of weighted average costing Fill the
	 * column ProcessedOn (if it exists) with a bigdecimal representation of
	 * current timestamp (with nanoseconds)
	 */
	public void setProcessedOn(String ColumnName, Object value, Object oldValue) {
		if ("Processed".equals(ColumnName) && value instanceof Boolean && ((Boolean) value).booleanValue() == true
				&& (oldValue == null
	...
	protected final boolean set_ValueNoCheck(String ColumnName, Object value) {
		...
		if (value == null)
			...
			// Set Boolean
				...
				m_newValues[index] = new Boolean("Y".equals(value));
			else if (p_info.getColumnClass(index) == Integer.class && value.getClass() == String.class) {
				try {
					int intValue = Integer.parseInt((String) value);
					m_newValues[index] = Integer.valueOf(intValue);
				} catch (Exception e) {
					log.warning(ColumnName + " - Class invalid: " + value.getClass().toString() + ", Should be "
							+ p_info.getColumnClass(index).toString() + ": " + value);
	...
	public final boolean set_CustomColumnReturningBoolean(String columnName, Object value) {
		...
		// this is for columns not in PO - verify and call proper method if
		// exists
	...
	public static void copyValues(PO from, PO to) {
		...
		if (from.getClass() != to.getClass()) {
			for (int i1 = 0; i1 < from.m_oldValues.length; i1++) {
				...
				if (colName.startsWith("Created") || colName.startsWith("Updated") || colName.equals("IsActive")
		...
		for (index = 0; index < size; index++) {
			...
			try {
				...
				if (p_info.isVirtualColumn(index)) // if rs constructor used
					...
					log.log(Level.SEVERE, "(rs) - " + String.valueOf(index) + ": " + p_info.getTableName() + "."
							+ p_info.getColumnName(index) + " (" + p_info.getColumnClass(index) + ") - " + e);
					success = false;
				}
			}
		}
		...
		for (index = 0; index < size; index++) {
			...
			try {
				...
				if (p_info.isVirtualColumn(index)) // if rs constructor used
					...
					log.log(Level.SEVERE, "(ht) - " + String.valueOf(index) + ": " + p_info.getTableName() + "."
							+ p_info.getColumnName(index) + " (" + p_info.getColumnClass(index) + ") - " + e);
					success = false;
				}
			}
	...
	/**
	 * Get Translation of column (if needed). It checks if the base language is
	 * used or the column is not translated. If there is no translation then it
	...
	public String get_Translation(String columnName, String AD_Language) {
		//
		// Check if columnName, AD_Language is valid or table support
		// translation (has 1 PK) => error
		if (columnName == null || AD_Language == null || m_IDs.length > 1 || m_IDs[0].equals(I_ZERO)
		...
		//
		// Check if NOT base language and column is translated => load trl from
		// db
		if (!Env.isBaseLanguage(AD_Language, get_TableName())
			...
			StringBuffer sql = new StringBuffer("SELECT ").append(columnName).append(" FROM ")
					.append(p_info.getTableName()).append("_Trl WHERE ").append(m_KeyColumns[0]).append("=?")
	 ...
	 * org.compiere.process.DocActionTemplate org.compiere.model.MClient
	 * org.compiere.model.MClientInfo org.compiere.model.MSystem
	 */
	/**************************************************************************
	...
	public boolean save() {
		...
		// Organization Check
		if (getAD_Org_ID() == 0 && (get_AccessLevel() == ACCESSLEVEL_ORG || (get_AccessLevel() == ACCESSLEVEL_CLIENTORG
		...
		if (getAD_Org_ID() != 0) {
			...
			if (!reset && MClientShare.isClientLevelOnly(getAD_Client_ID(), get_Table_ID())) {
				reset = get_AccessLevel() == ACCESSLEVEL_CLIENT || get_AccessLevel() == ACCESSLEVEL_SYSTEMCLIENT
						|| get_AccessLevel() == ACCESSLEVEL_ALL || get_AccessLevel() == ACCESSLEVEL_CLIENTORG;
			}
			if (reset) {
		...
		if (m_trxName == null) {
			...
			if (trx == null) {
				...
				// Creating and starting the transaction right here, but please
				// note
	...
	private boolean saveFinish(boolean newRecord, boolean success) {
		...
		// Call ModelValidators TYPE_AFTER_NEW/TYPE_AFTER_CHANGE - teo_sarca [
		// 1675490 ]
		if (success && !isDirectLoad) {
			String errorMsg = ModelValidationEngine.get().fireModelChange(this, newRecord
					? (isReplication() ? ModelValidator.TYPE_AFTER_NEW_REPLICATION : ModelValidator.TYPE_AFTER_NEW)
					: (isReplication() ? ModelValidator.TYPE_AFTER_CHANGE_REPLICATION
	...
	protected boolean beforeSave(boolean newRecord) {
		/**
		 * Prevents saving log.saveError("Error", Msg.parseTranslation(getCtx(),
		 * "@C_Currency_ID@ = @C_Currency_ID@")); log.saveError("FillMandatory",
		 * Msg.getElement(getCtx(), "PriceEntered")); /** Issues message
		 * log.saveWarning(AD_Message, message); log.saveInfo (AD_Message,
		 * message);
		 **/
		return true;
	} // beforeSave
	...
	protected boolean saveUpdate() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (value == Null.NULL)
				...
				sql.append(
						DB.TO_DATE((Timestamp) encrypt(i, value), p_info.getColumnDisplayType(i) == DisplayType.Date));
			else {
				if (value.toString().length() == 0) {
					// [ 1722057 ] Encrypted columns throw error if saved as
					// null
					// don't encrypt NULL
			...
			// Change Log - Only
			if (session != null && m_IDs.length == 1 && p_info.isAllowLogging(i) // logging
																					// allowed
				...
				MChangeLog cLog = session.changeLog(m_trxName, AD_ChangeLog_ID, p_info.getAD_Table_ID(),
						p_info.getColumn(i).AD_Column_ID, get_ID(), getAD_Client_ID(), getAD_Org_ID(), oldV, newV,
						MChangeLog.EVENTCHANGELOG_Update);
				if (cLog != null)
		...
		if (changes) {
			...
			if (ok)
				...
				if (m_trxName == null)
					log.log(Level.WARNING, "#" + no + " - " + p_info.getTableName() + "." + where);
				else
					log.log(Level.WARNING,
							"#" + no + " - [" + m_trxName + "] - " + p_info.getTableName() + "." + where);
			}
	...
	private boolean saveNew() {
		...
		// previously
		if (m_IDs.length == 1 && p_info.hasKeyColumn() && m_KeyColumns[0].endsWith("_ID") && !isDirectLoad) // AD_Language,
																											// EntityType
		{
		...
		for (int i = 0; i < size; i++) {
			...
			Class<?> c = p_info.getColumnClass(i);
			try {
				if (c == Object.class) // may have need to deal with null values
										// differently
					...
					sqlValues.append(DB.TO_DATE((Timestamp) encrypt(i, value),
			...
			if (session != null && m_IDs.length == 1 && p_info.isAllowLogging(i) // logging
					...
					&& !p_info.isEncrypted(i) // not encrypted
					&& !p_info.isVirtualColumn(i) // no virtual column
					&& !"Password".equals(columnName) && (insertLog.equalsIgnoreCase("Y")
							|| (insertLog.equalsIgnoreCase("K") && p_info.getColumn(i).IsKey))) {
				// change log on new
				MChangeLog cLog = session.changeLog(m_trxName, AD_ChangeLog_ID, p_info.getAD_Table_ID(),
						p_info.getColumn(i).AD_Column_ID, get_ID(), getAD_Client_ID(), getAD_Org_ID(), null, value,
						MChangeLog.EVENTCHANGELOG_Insert);
				if (cLog != null)
					AD_ChangeLog_ID = cLog.getAD_ChangeLog_ID();
			}
	...
	/**
	 * Get ID for new record during save. You can overwrite this to explicitly
	 * set the ID
	 * 
	...
	public boolean delete(boolean force) {
		...
		try {
			...
			errorMsg = ModelValidationEngine.get().fireModelChange(this,
			...
			StringBuffer sql = new StringBuffer("DELETE FROM ") // jz why no
																// FROM??
					.append(p_info.getTableName()).append(" WHERE ").append(get_WhereClause(true));
			int no = 0;
			if (isUseTimeoutForUpdate())
					...
					if (p_info.isChangeLog()) {
						...
						if (session != null && m_IDs.length == 1) {
							...
							for (int i = 0; i < size; i++) {
								...
								if (value != null && p_info.isAllowLogging(i) // logging
																				// allowed
										&& !p_info.isEncrypted(i) // not
																	// encrypted
										&& !p_info.isVirtualColumn(i) // no
																		// virtual
																		// column
										&& !"Password".equals(p_info.getColumnName(i))) {
									// change log on delete
									MChangeLog cLog = session.changeLog(m_trxName != null ? m_trxName : localTrxName,
											AD_ChangeLog_ID, AD_Table_ID, p_info.getColumn(i).AD_Column_ID, Record_ID,
											getAD_Client_ID(), getAD_Org_ID(), value, null,
											MChangeLog.EVENTCHANGELOG_Delete);
									if (cLog != null)
										AD_ChangeLog_ID = cLog.getAD_ChangeLog_ID();
								}
							} // for all fields
	...
	public boolean insertTranslations() {
		...
		StringBuffer sql = new StringBuffer("INSERT INTO ").append(tableName).append("_Trl (AD_Language,")
				.append(keyColumn).append(", ").append(iColumns)
				...
				.append(keyColumn).append("=").append(get_ID()).append(" AND NOT EXISTS (SELECT * FROM ")
				.append(tableName).append("_Trl tt WHERE tt.AD_Language=l.AD_Language AND tt.").append(keyColumn)
				.append("=t.").append(keyColumn).append(")");
	...
	private boolean updateTranslations() {
		...
		for (int i = 0; i < p_info.getColumnCount(); i++) {
			if (p_info.isColumnTranslated(i) && is_ValueChanged(p_info.getColumnName(i))) {
	...
	private boolean deleteTranslations(String trxName) {
		...
		String tableName = p_info.getTableName();
		String keyColumn = m_KeyColumns[0];
		StringBuffer sql = new StringBuffer("DELETE  FROM  ").append(tableName).append("_Trl WHERE ").append(keyColumn)
	...
	protected boolean insert_Accounting(String acctTable, String acctBaseTable, String whereClause) {
		...
		StringBuffer sb = new StringBuffer("INSERT INTO ").append(acctTable).append(" (").append(get_TableName())
		...
		// .. SELECT
		sb.append(") SELECT ").append(get_ID()).append(", p.C_AcctSchema_ID, p.AD_Client_ID,0,'Y', SysDate,")
				.append(getUpdatedBy()).append(",SysDate,").append(getUpdatedBy());
		for (int i = 0; i < s_acctColumns.size(); i++)
			sb.append(",p.").append(s_acctColumns.get(i));
		// .. FROM
		sb.append(" FROM ").append(acctBaseTable).append(" p WHERE p.AD_Client_ID=").append(getAD_Client_ID());
		if (whereClause != null && whereClause.length() > 0)
			sb.append(" AND ").append(whereClause);
		sb.append(" AND NOT EXISTS (SELECT * FROM ").append(acctTable)
				.append(" e WHERE e.C_AcctSchema_ID=p.C_AcctSchema_ID AND e.").append(get_TableName()).append("_ID=")
	...
	protected boolean insert_Tree(String treeType, int C_Element_ID) {
		...
		final StringBuilder select = new StringBuilder("SELECT t.AD_Tree_ID FROM AD_Tree t WHERE t.AD_Client_ID=")
				.append(getAD_Client_ID()).append(" AND t.IsActive='Y'");
		if (C_Element_ID != 0)
			select.append(" AND EXISTS (SELECT * FROM C_Element ae WHERE ae.C_Element_ID=").append(C_Element_ID)
	...
	public boolean lock() {
		...
		if (index != -1) {
			...
			String sql = "UPDATE " + p_info.getTableName()
					+ " SET Processing='Y' WHERE (Processing='N' OR Processing IS NULL) AND " + get_WhereClause(true);
			boolean success = false;
			if (isUseTimeoutForUpdate())
				success = DB.executeUpdateEx(sql, null, QUERY_TIME_OUT) == 1; // outside
																				// trx
			else
	...
	public boolean unlock(String trxName) {
		...
		if (index != -1) {
			...
			String sql = "UPDATE " + p_info.getTableName() + " SET Processing='N' WHERE " + get_WhereClause(true);
			boolean success = false;
			if (isUseTimeoutForUpdate())
	...
	public void dump(int index) {
		...
		sb.append(": ").append(get_ColumnName(index)).append(" = ").append(m_oldValues[index]).append(" (")
				.append(m_newValues[index]).append(")");
	...
	private void lobAdd(Object value, int index, int displayType) {
		...
		PO_LOB lob = new PO_LOB(p_info.getTableName(), get_ColumnName(index), get_WhereClause(true), displayType,
				value);
		if (m_lobInfo == null)
	...
	/*
	 * Doc - To be used on ModelValidator to get the corresponding Doc from the
	 * PO
	 */
	private Doc m_doc;

	/**
	 * Set the accounting document associated to the PO - for use in POST
	 * ModelValidator
	 * 
	 * @param doc
	 *            Document
