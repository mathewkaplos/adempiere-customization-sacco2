...
public class X_l_repayments extends PO implements I_l_repayments, I_Persistent {
	...
	public X_l_repayments(Properties ctx, int l_repayments_ID, String trxName) {
		...
		/**
		 * if (l_repayments_ID == 0) { setbankgl_Acct (0); setIsComplete (false); // N
		 * setl_repayments_ID (0); setLoanNo (null); setPosted (false); // N
		 * setProcessed (false); // N setReversed (false); // N }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_l_repayments[").append(get_ID()).append("]");
		return sb.toString();
	}
	...
	public I_C_ValidCombination getinterestgl_A() throws RuntimeException {
		return (I_C_ValidCombination) MTable.get(getCtx(), I_C_ValidCombination.Table_Name).getPO(getinterestgl_Acct(),
				get_TrxName());
	}
	...
	public I_C_ValidCombination getloan_gl_A() throws RuntimeException {
		return (I_C_ValidCombination) MTable.get(getCtx(), I_C_ValidCombination.Table_Name).getPO(getloan_gl_Acct(),
				get_TrxName());
	}
	...
	public I_s_loans gets_loans() throws RuntimeException {
		return (I_s_loans) MTable.get(getCtx(), I_s_loans.Table_Name).getPO(gets_loans_ID(), get_TrxName());
	}
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		// newRecord
		if (newRecord) {
			String sql = "SELECT COUNT(*)  FROM adempiere.l_repayments WHERE isComplete='N' AND s_loans_id = "
					+ gets_loans_ID();
			int x = DB.getSQLValue(get_TrxName(), sql);
			if (x > 0) {
				ADialog.error(2, null,
						"You have a pending record to complete. Please complete it first or you remove it!. Undo this record.");
				return false;
			}
		}
