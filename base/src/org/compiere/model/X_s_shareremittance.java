...
public class X_s_shareremittance extends PO implements I_s_shareremittance, I_Persistent {
	...
	public X_s_shareremittance(Properties ctx, int s_shareremittance_ID, String trxName) {
		...
		/**
		 * if (s_shareremittance_ID == 0) { setDocStatus (null); // DR setisfixeddeposit
		 * (false); // N setPosted (false); // N setProcessed (false); // N
		 * setremittancedate (new Timestamp( System.currentTimeMillis() )); // @Date@
		 * setReversed (false); // N sets_member_ID (0); sets_membershares_ID (0);
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_s_shareremittance[").append(get_ID()).append("]");
		return sb.toString();
	}
	...
	public I_C_ValidCombination getbankgl_A() throws RuntimeException {
		return (I_C_ValidCombination) MTable.get(getCtx(), I_C_ValidCombination.Table_Name).getPO(getbankgl_Acct(),
				get_TrxName());
	}
	...
	public I_C_ValidCombination getsharegl_A() throws RuntimeException {
		return (I_C_ValidCombination) MTable.get(getCtx(), I_C_ValidCombination.Table_Name).getPO(getsharegl_Acct(),
				get_TrxName());
	}
	...
	public I_s_member gets_member() throws RuntimeException {
		return (I_s_member) MTable.get(getCtx(), I_s_member.Table_Name).getPO(gets_member_ID(), get_TrxName());
	}
	...
	public I_s_payment_mode gets_payment_mode() throws RuntimeException {
		return (I_s_payment_mode) MTable.get(getCtx(), I_s_payment_mode.Table_Name).getPO(gets_payment_mode_ID(),
				get_TrxName());
	}
	...
	public I_s_sharetype gets_sharetype() throws RuntimeException {
		return (I_s_sharetype) MTable.get(getCtx(), I_s_sharetype.Table_Name).getPO(gets_sharetype_ID(), get_TrxName());
	}
	 ...
	 * Set Supervisor.
	 * 
	 * @param Supervisor_ID Supervisor for this user/organization - used for
	 ...
	 * Get Supervisor.
	 * 
	 * @return Supervisor for this user/organization - used for escalation and
	 *         approval
	 */
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		if (newRecord) {
			String sql = "SELECT COUNT(*)  FROM adempiere.s_shareremittance WHERE isComplete='N' AND s_member_id = "
					+ gets_member_ID();
			int x = DB.getSQLValue(get_TrxName(), sql);
			if (x > 0) {
				ADialog.error(2, null,
						"You have a pending record to complete. Please complete it first or you remove it!."
								+ " Undo this record. If you can't see another record, "
								+ "then check on either deposit or withdrawal window to see if there's a pending record for this member "
								+ " then remove it or complete it!");
				return false;
			}
		}
