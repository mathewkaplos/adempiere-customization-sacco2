...
public final class FactLine extends X_Fact_Acct {
	public FactLine(Properties ctx, ResultSet rs, String trxName) {
	...
	public FactLine(Properties ctx, int AD_Table_ID, int Record_ID, int Line_ID, String trxName) {
		...
		// Log.trace(this,Log.l1_User, "FactLine " + AD_Table_ID + ":" +
		// Record_ID);
		...
		if (newRecord) {
			...
			if (m_doc.getDocumentType().equals(Doc.DOCTYPE_ARInvoice) && m_docLine != null
				...
				setAccount_ID(createRevenueRecognition(m_docLine.getC_RevenueRecognition_ID(), m_docLine.get_ID(),
						...
						getUser2_ID(), getUserElement1_ID(), getUserElement2_ID()));
			}
		}
		return true;
	} // beforeSave
	...
	/**************************************************************************
	 * Update Line with reversed Original Amount in Accounting Currency. Also
	 * copies original dimensions like Project, etc. Called from Doc_MatchInv
	 * 
	 * @param AD_Table_ID
	 *            table
	 * @param Record_ID
	...
	public boolean updateReverseLine(int AD_Table_ID, int Record_ID, int Line_ID, BigDecimal quantity,
		...
		try {
			...
			if (rs.next()) {
				...
				setAmtSource(fact.getC_Currency_ID(), crSourceAmt.multiply(multiplier),
