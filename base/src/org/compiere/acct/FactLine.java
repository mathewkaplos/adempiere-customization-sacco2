 ...
 * 
 *          Contributor(s): Chris Farley: Fix Bug [ 1657372 ] M_MatchInv records
 *          can not be balanced
 *          https://sourceforge.net/forum/message.php?msg_id=4151117 Carlos Ruiz
 *          - globalqss: Add setAmtAcct method rounded by Currency Armen Rizal,
 *          Goodwill Consulting
 *          <li>BF [ 1745154 ] Cost in Reversing Material Related Docs Bayu
 *          Sistematika -
 *          <li>BF [ 2213252 ] Matching Inv-Receipt generated unproperly value
 *          for src amt Teo Sarca
...
public final class FactLine extends X_Fact_Acct {
	...
	public void setAccount(MAcctSchema acctSchema, MAccount acct) {
		...
		MAcctSchemaElement acctSchemaElement1 = m_acctSchema
		...
		MAcctSchemaElement acctSchemaElement2 = m_acctSchema
				.getAcctSchemaElement(X_C_AcctSchema_Element.ELEMENTTYPE_UserElement2);
	...
	public boolean setAmtSource(int C_Currency_ID, BigDecimal AmtSourceDr, BigDecimal AmtSourceCr) {
		...
		// if (getAmtSourceDr().compareTo(Env.ZERO)==0 &&
		// getAmtSourceCr().compareTo(Env.ZERO)==0)
		// return false;
	...
	public void setDocumentInfo(Doc doc, DocLine docLine) {
		...
		// Date Trx
		if (m_doc.getDateDoc() == null)
			setDateTrx(DateUtil.newTimestamp());
		else
			setDateTrx(m_doc.getDateDoc());
		if (m_docLine != null && m_docLine.getDateDoc() != null)
			setDateTrx(m_docLine.getDateDoc());
		// Date Acct
		if (m_doc.getDateAcct() == null)
			setDateAcct(DateUtil.newTimestamp());
		else
		...
		else if (m_doc.getC_Period_ID() > 1)
		...
		setReferenceNo(m_doc.getDocumentNo());
	...
	public void currencyCorrect(BigDecimal deltaAmount) {
		...
		log.fine(deltaAmount.toString() + "; Old-AcctDr=" + getAmtAcctDr() + ",AcctCr=" + getAmtAcctCr() + "; Negative="
				+ negative + "; AdjustDr=" + adjustDr);
	...
	public boolean convert() {
		...
		setAmtAcctDr(
				MConversionRate.convert(getCtx(), getAmtSourceDr(), getC_Currency_ID(), m_acctSchema.getC_Currency_ID(),
						getDateAcct(), C_ConversionType_ID, m_doc.getAD_Client_ID(), AD_Org_ID));
		if (getAmtAcctDr() == null)
			return false;
		setAmtAcctCr(
				MConversionRate.convert(getCtx(), getAmtSourceCr(), getC_Currency_ID(), m_acctSchema.getC_Currency_ID(),
						getDateAcct(), C_ConversionType_ID, m_doc.getAD_Client_ID(), AD_Org_ID));
		return true;
	} // convert
	...
	public String toString() {
		...
		sb.append(getAD_Table_ID()).append(":").append(getRecord_ID()).append(",").append(m_acct).append(",Cur=")
				.append(getC_Currency_ID()).append(", DR=").append(getAmtSourceDr()).append("|").append(getAmtAcctDr())
				.append(", CR=").append(getAmtSourceCr()).append("|").append(getAmtAcctCr()).append("]");
		return sb.toString();
	} // toString

	/**
	 * Get AD_Org_ID (balancing segment). (if not set directly - from document line,
	 * document, account, locator)
	 * <p>
	 * Note that Locator needs to be set before - otherwise segment balancing might
	...
	public int getC_SalesRegion_ID() {
		...
		if (m_doc != null) {
			...
			if (super.getC_SalesRegion_ID() == 0 && m_doc.getC_BPartner_Location_ID() != 0
	...
	protected boolean beforeSave(boolean newRecord) {
		if (newRecord) {
			...
			if (m_doc.getDocumentType().equals(Doc.DOCTYPE_ARInvoice) && m_docLine != null
					&& m_docLine.getC_RevenueRecognition_ID() != 0) {
				int AD_User_ID = 0;
				setAccount_ID(createRevenueRecognition(m_docLine.getC_RevenueRecognition_ID(), m_docLine.get_ID(),
						getAD_Client_ID(), getAD_Org_ID(), AD_User_ID, getAccount_ID(), getC_SubAcct_ID(),
						getM_Product_ID(), getC_BPartner_ID(), getAD_OrgTrx_ID(), getC_LocFrom_ID(), getC_LocTo_ID(),
						getC_SalesRegion_ID(), getC_Project_ID(), getC_Campaign_ID(), getC_Activity_ID(), getUser1_ID(),
						getUser2_ID(), getUser3_ID(), getUser3_ID(), getUserElement1_ID(), getUserElement2_ID()));
			}
		}
		return true;
	} // beforeSave
	...
	/**************************************************************************
	 * Revenue Recognition. Called from FactLine.save
	 * <p>
	 * Create Revenue recognition plan and return Unearned Revenue account to be
	 * used instead of Revenue Account. If not found, it returns the revenue
	 * account.
	 *
	...
	private int createRevenueRecognition(int C_RevenueRecognition_ID, int C_InvoiceLine_ID, int AD_Client_ID,
			int AD_Org_ID, int AD_User_ID, int Account_ID, int C_SubAcct_ID, int M_Product_ID, int C_BPartner_ID,
			int AD_OrgTrx_ID, int C_LocFrom_ID, int C_LocTo_ID, int C_SRegion_ID, int C_Project_ID, int C_Campaign_ID,
			int C_Activity_ID, int User1_ID, int User2_ID, int UserElement1_ID, int UserElement2_ID) {
		return createRevenueRecognition(C_RevenueRecognition_ID, C_InvoiceLine_ID, AD_Client_ID, AD_Org_ID, AD_User_ID,
				Account_ID, C_SubAcct_ID, M_Product_ID, C_BPartner_ID, AD_OrgTrx_ID, C_LocFrom_ID, C_LocTo_ID,
				C_SRegion_ID, C_Project_ID, C_Campaign_ID, C_Activity_ID, User1_ID, User2_ID, 0, 0, UserElement1_ID,
				UserElement2_ID);
	}
	...
	private int createRevenueRecognition(int C_RevenueRecognition_ID, int C_InvoiceLine_ID, int AD_Client_ID,
			int AD_Org_ID, int AD_User_ID, int Account_ID, int C_SubAcct_ID, int M_Product_ID, int C_BPartner_ID,
			int AD_OrgTrx_ID, int C_LocFrom_ID, int C_LocTo_ID, int C_SRegion_ID, int C_Project_ID, int C_Campaign_ID,
			int C_Activity_ID, int User1_ID, int User2_ID, int User3_ID, int User4_ID, int UserElement1_ID,
			int UserElement2_ID) {
		log.fine("From Accout_ID=" + Account_ID);
		// get VC for P_Revenue (from Product)
		MAccount revenue = MAccount.get(getCtx(), AD_Client_ID, AD_Org_ID, getC_AcctSchema_ID(), Account_ID,
				C_SubAcct_ID, M_Product_ID, C_BPartner_ID, AD_OrgTrx_ID, C_LocFrom_ID, C_LocTo_ID, C_SRegion_ID,
				C_Project_ID, C_Campaign_ID, C_Activity_ID, User1_ID, User2_ID, User3_ID, User4_ID, UserElement1_ID,
		...
		String sql = "SELECT ga.UnearnedRevenue_Acct, vc.Account_ID "
				...
				+ "WHERE ga.C_BP_Group_ID=p.C_BP_Group_ID" + " AND ga.UnearnedRevenue_Acct=vc.C_ValidCombination_ID"
		...
		log.fine("From Acctount_ID=" + Account_ID + " to " + new_Account_ID + " - Plan from UnearnedRevenue_Acct="
				+ UnearnedRevenue_Acct + " to Revenue_Acct=" + P_Revenue_Acct);
		return new_Account_ID;
	} // createRevenueRecognition

	/**************************************************************************
	 * Update Line with reversed Original Amount in Accounting Currency. Also copies
	 * original dimensions like Project, etc. Called from Doc_MatchInv
	 * 
	 ...
	 */
	public boolean updateReverseLine(int AD_Table_ID, int Record_ID, int Line_ID, BigDecimal quantity,
			BigDecimal multiplier) {
		boolean success = false;

		String sql = "SELECT * " + "FROM Fact_Acct " + "WHERE C_AcctSchema_ID=? AND AD_Table_ID=? AND Record_ID=?"
		...
		try {
			...
			if (rs.next()) {
				...
				setAmtSource(fact.getC_Currency_ID(), drSourceAmt.multiply(multiplier),
						crSourceAmt.multiply(multiplier));
				// end Bayu Sistematika
				//
				success = true;
				log.fine(new StringBuffer("(Table=").append(AD_Table_ID).append(",Record_ID=").append(Record_ID)
						.append(",Line=").append(Record_ID).append(", Account=").append(m_acct).append(",dr=")
						.append(dr).append(",cr=").append(cr).append(") - DR=").append(getAmtSourceDr()).append("|")
						.append(getAmtAcctDr()).append(", CR=").append(getAmtSourceCr()).append("|")
						.append(getAmtAcctCr()).toString());
				// Dimensions
				...
				log.warning(new StringBuffer("Not Found (try later) ").append(",C_AcctSchema_ID=")
						.append(getC_AcctSchema_ID()).append(", AD_Table_ID=").append(AD_Table_ID).append(",Record_ID=")
						.append(Record_ID).append(",Line_ID=").append(Line_ID).append(", Account_ID=")
						.append(m_acct.getAccount_ID()).toString());
		} catch (SQLException e) {
			log.log(Level.SEVERE, sql, e);
		} finally {
			DB.close(rs, pstmt);
