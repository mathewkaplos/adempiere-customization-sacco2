...
public final class Adempiere {
	...
	public static void main(String[] args) {
		...
		// start checking for last seen
		System.out.println("start checking for last seen");
		Env.updateLastSeen();

		final long idleTimeInterval = Sacco.getSaccco().getsystemidletime() * 60 * 1000;
		try {
			while (true) {
				long idleTime = System.currentTimeMillis() - Env.getLastSeen().getTime();
				if (idleTimeInterval < idleTime) {
					AMenu aMenu = (AMenu) Env.getWindow(0);
					aMenu.logout();
				}
				Thread.sleep(10 * 1 * 1000); // 10 seconds
